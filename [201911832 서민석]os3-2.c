...
int demandPaging(process_raw* data, pte* cur_pte, int i, int* frame_index)
{
    list_for_each_entry(data, &job_q, job)
    {
        ...
        int L1PT_index = ref / 8;
        int L2PT_index = ref % 8;
        int index = 0;

        if (data->ref_len - i > 0)
        {
            // L1PT
            index = L1PT_index + data->pid * 8;

            //printf("[PID %02d REF:%03d] Page access %03d: ", data->pid, i, ref);
            //printf("(L1PT) ");
            //printf("index %d vflag %d ", index, cur_pte[index].vflag);
            if (cur_pte[index].vflag == PAGE_INVALID)
            {
                ...
                cur_pte[index].ref++;
                //printf("PF,Allocated Frame %03d -> %03d,", L1PT_index, *frame_index);
                *frame_index = *frame_index + 1;
            }
            else if (cur_pte[index].vflag == PAGE_VALID)
            {
                cur_pte[index].ref++;
                //printf("Frame %03d,", cur_pte[index].frame);
            }

            // L2PT
            index = L2PT_index + cur_pte[index].frame * PAGETABLE_FRAMES; // L1PT가 가리키는 frame의 pte
...
void osSimulator(process_raw* data)
{
    ...
    list_for_each_entry(data, &job_q, job)
    {
        int L1PT_start = 8 * data->pid; // L1PT for() starting point
        int L2PT_start = 0; // L2PT for() starting poind
        // count page faults & frames
        int pf = 0;
        int alloc_frames = 1;

        for (int i = L1PT_start; i < L1PT_start + 8; i++)
        {
            if (cur_pte[i].vflag == PAGE_VALID && cur_pte[i].ref != 0)
            {
                pf++;
                alloc_frames++;
                L2PT_start = PAGETABLE_FRAMES * cur_pte[i].frame;
                for (int j = L2PT_start; j < L2PT_start + PAGETABLE_FRAMES; j++)
                {
                    if (cur_pte[j].vflag == PAGE_VALID && cur_pte[j].ref != 0)
                    {
                        pf++;
                        alloc_frames++;
                    }
            ...
            if (cur_pte[i].vflag == PAGE_VALID && cur_pte[i].ref != 0)
            {
                ...
                for (int j = L2PT_start; j < L2PT_start + PAGETABLE_FRAMES; j++)
                {
                    if (cur_pte[j].vflag == PAGE_VALID && cur_pte[j].ref != 0)
                    {
                        ...
                        j - L2PT_start + (i - L1PT_start) * 8, cur_pte[j].frame, cur_pte[j].ref);
                    }
                }
            }
        }
